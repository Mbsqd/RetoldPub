...
interface DataTableProps<TData extends { id: number; typeWeek: number; day: number; timeStart: string }, TValue> {
...
export function DataTable<TData extends { id: number; typeWeek: number; day: number; timeStart: string }, TValue>({
	columns,
	data,
	blockId,
	fetchScheduleData,
	...
	useEffect(() => {
		const sortedData = [...data].sort((a, b) => {
			// Сортировка по типу недели (0 сначала, потом 1, потом 2)
			if (a.typeWeek !== b.typeWeek) {
				return a.typeWeek - b.typeWeek; // Это будет сортировать 0 < 1 < 2
			}
	...
	const handleDelete = async (id: number) => {
		try {
			const response = await fetch(`/api/Blocks/${blockId}/delete-schedule`, {
				method: "DELETE",
				headers: {
	...
	const handleEdit = async () => {
		...
		try {
			const response = await fetch(`/api/Blocks/${blockId}/update-schedule`, {
				method: "PUT",
				headers: {
					"Content-Type": "application/json",
				},
			...
			// Обновляем данные непосредственно в tableData
	...
	const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>, field: keyof TData) => {
		if (editedRowData) {
			...
			if (field === "lessonType" && value.length > 64) value = value.slice(0, 64);
			if (field === "cabinet" && value.length > 5) value = value.slice(0, 5);
			if (field === "linkFirst" && value.length > 256) value = value.slice(0, 256);
			if (field === "linkSecond" && value.length > 256) value = value.slice(0, 256);
	...
	const handleSelectChange = (value: string, field: keyof TData) => {
		const parsedValue = field === "typeWeek" ? Number(value) : Number(value);
		if (editedRowData) {
			setEditedRowData({
				...editedRowData,
				[field]: parsedValue,
												...
												return (
													<TableCell key={cell.id} className="relative">
														{editingRowId === row.original.id ? (
															field === "typeWeek" ? (
																<Select
                                                                    value={String((editedRowData as any)[field])}
                                                                ...
                                                                >
                                                                    ...
                                                                    <SelectContent>
                                                                        {Object.entries(TypeWeekMap).map(([key, value]) => (
